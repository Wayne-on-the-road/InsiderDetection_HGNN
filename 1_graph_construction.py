...
def onehot_data(data):
    ...
    x_columns= data.columns[-74:]
    x = data[x_columns]
    x = np.array(x)
    x = torch.LongTensor(x)
    # print(x)
    # print(type(x))
    x = F.one_hot(x,num_classes=25)
    x = x.float()
    # print(x)
...
def generate_supervision_relationships(data_path, userlist_path):
    ...
    #     'user_ID': user_IDs})
    UserDay_df = pd.DataFrame({'UserDay_index': head_nodes})
    UserDay_df = pd.concat([UserDay_df, data[['date_index','user_index','group','label','f1','f2','f3','f4','f5']],data.iloc[:, -74:]],axis=1)
...
data['UserDay'].x = torch.tensor(UserDay_df[['f1', 'f2', 'f3', 'f4', 'f5']].values, dtype=torch.float64) # manual feature
sequence_onehot = onehot_data(UserDay_df)
data['UserDay'].s = sequence_onehot # sequence feature
